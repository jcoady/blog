...
A note on notation, carried forward from this point in the series: capital $Q$ denotes a Rational Trigonometry quadrance — the raw value of the ambient quadratic form on a single homogeneous representative — which is not invariant under choice of representative. Lowercase $q$ denotes a UHG quadrance — the projectively invariant ratio between two points. The trace-free triple below, $q_1,q_2,q_3$, is built entirely from quadrances of this second kind, so it is labeled accordingly.
...
Pick one representative from each of the three complementary pairs in $\mathcal{O}_q$ — say $q_a = q$, $q_b = 1/q$, $q_c = 1/(1-q)$ — and feed each one through
...
# --- SU(3)-flavored central field components (UHG quadrances) ---
q1 = (q**2 - q + 1) / q
q2 = (q**2 - q + 1) / (q*(q - 1))
q3 = -(q**2 - q + 1) / (q - 1)
